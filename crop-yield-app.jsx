...
// ─────────────────────────────────────────
const MOCK_HISTORY = [
  { id: 1, date: "2025-03-10", crop: "Maize", region: "Kano", soilType: "Loamy", rainfall: 820, temperature: 28, humidity: 65, fertilizer: 120, pesticide: 4.5, area: 3.5, season: "Wet", year: 2025, predictedYield: 4.72, unit: "tons/ha", status: "Completed" },
  { id: 2, date: "2025-03-08", crop: "Rice", region: "Benue", soilType: "Clay", rainfall: 1100, temperature: 26, humidity: 78, fertilizer: 180, pesticide: 6.2, area: 5.0, season: "Wet", year: 2025, predictedYield: 6.14, unit: "tons/ha", status: "Completed" },
  { id: 3, date: "2025-03-05", crop: "Sorghum", region: "Sokoto", soilType: "Sandy", rainfall: 450, temperature: 34, humidity: 42, fertilizer: 80, pesticide: 2.1, area: 2.0, season: "Dry", year: 2025, predictedYield: 2.88, unit: "tons/ha", status: "Completed" },
  { id: 4, date: "2025-02-28", crop: "Cassava", region: "Ogun", soilType: "Loamy", rainfall: 1350, temperature: 29, humidity: 80, fertilizer: 60, pesticide: 1.8, area: 4.2, season: "Wet", year: 2025, predictedYield: 18.3, unit: "tons/ha", status: "Completed" },
  { id: 5, date: "2025-02-20", crop: "Wheat", region: "Plateau", soilType: "Clay Loam", rainfall: 650, temperature: 22, humidity: 55, fertilizer: 220, pesticide: 7.5, area: 6.0, season: "Dry", year: 2024, predictedYield: 3.45, unit: "tons/ha", status: "Completed" },
];
...
const mockPredict = async (formData) => {
  await new Promise(r => setTimeout(r, 2200));
  const base = { Maize: 4.5, Rice: 5.8, Sorghum: 2.9, Cassava: 18.0, Wheat: 3.2, Yam: 12.0, Cowpea: 1.4, Groundnut: 2.1, Millet: 1.8, Sugarcane: 65, Tomato: 22, Cotton: 1.9 };
  const b = base[formData.crop] || 3.5;
  const rainfallFactor = (parseFloat(formData.rainfall) / 800) * 0.3;
  const fertFactor = (parseFloat(formData.fertilizer) / 150) * 0.2;
  const tempFactor = formData.temperature > 30 ? -0.1 : 0.05;
  const noise = (Math.random() - 0.5) * 0.4;
  const yield_ = +(b * (1 + rainfallFactor + fertFactor + tempFactor) + noise).toFixed(2);
  const confidence = +(78 + Math.random() * 15).toFixed(1);
  return { predictedYield: yield_, unit: "tons/ha", confidence, model: "Random Forest Regressor", r2Score: 0.891 };
};
...
const Input = ({ label, name, type = "text", value, onChange, placeholder, error, required, min, max, step, hint }) => (
  <div className="flex flex-col gap-1">
    ...
    <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} min={min} max={max} step={step}
      className={`w-full rounded-lg border px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition ${error ? "border-red-400" : "border-slate-300"}`} />
...
const Select = ({ label, name, value, onChange, options, error, required, hint }) => (
  <div className="flex flex-col gap-1">
    ...
    <select name={name} value={value} onChange={onChange}
      className={`w-full rounded-lg border px-3.5 py-2.5 text-sm text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition ${error ? "border-red-400" : "border-slate-300"}`}>
...
const SectionTitle = ({ eyebrow, title, desc }) => (
  <div className="text-center mb-12">
    {eyebrow && <p className="text-emerald-700 font-semibold text-xs uppercase tracking-widest mb-2">{eyebrow}</p>}
    <h2 className="text-3xl font-bold text-slate-900 mb-3">{title}</h2>
    {desc && <p className="text-slate-500 max-w-xl mx-auto text-base leading-relaxed">{desc}</p>}
  </div>
);
  ...
  <>
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-slate-200 shadow-sm">
      <div className="max-w-screen-xl mx-auto flex items-center justify-between px-4 md:px-8 h-16">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setPage(user ? "dashboard" : "landing")}>
          ...
          <div>
            <p className="font-bold text-slate-900 text-sm leading-tight">CropYield<span className="text-emerald-700">AI</span></p>
            <p className="text-xs text-slate-400 leading-tight hidden sm:block">Prediction System</p>
          </div>
        </div>
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {NAV_LINKS.map(l => (
              <button key={l.page} onClick={() => setPage(l.page)}
                className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-medium transition-all ${page === l.page ? "bg-emerald-50 text-emerald-800" : "text-slate-600 hover:bg-slate-100"}`}>
            ...
            <>
              <div className="hidden sm:flex items-center gap-2 text-sm text-slate-600">
                <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center">
                  <Icon path={Icons.user} size={14} className="text-emerald-700" />
                </div>
                <span className="font-medium text-slate-700 hidden md:inline">{user.name}</span>
              </div>
              <button onClick={onLogout} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-red-600 transition px-2 py-1.5 rounded-lg hover:bg-red-50">
    ...
    {user && sidebarOpen && (
      <div className="lg:hidden fixed inset-0 z-30 flex">
        ...
        <div className="relative z-10 w-64 bg-white h-full shadow-2xl flex flex-col pt-4">
          ...
          {NAV_LINKS.map(l => (
            <button key={l.page} onClick={() => { setPage(l.page); setSidebarOpen(false); }}
              className={`flex items-center gap-3 px-5 py-3 text-sm font-medium transition ${page === l.page ? "bg-emerald-50 text-emerald-800 border-r-4 border-emerald-700" : "text-slate-600 hover:bg-slate-50"}`}>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="relative bg-gradient-to-br from-emerald-900 via-emerald-800 to-slate-900 text-white overflow-hidden">
        ...
        <div className="relative max-w-screen-xl mx-auto px-6 md:px-8 pt-20 pb-24 md:pt-28 md:pb-32 grid md:grid-cols-2 gap-12 items-center">
          <div>
            <div className="inline-flex items-center gap-2 bg-emerald-700/50 border border-emerald-600/40 rounded-full px-4 py-1.5 text-xs font-semibold text-emerald-200 mb-6">
      ...
      <section className="py-16 bg-white">
        <div className="max-w-screen-xl mx-auto px-6 md:px-8">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <h2 className="text-3xl font-bold text-slate-900 mb-5">What is Crop Yield Prediction?</h2>
      ...
      <section className="py-16 bg-slate-50">
        <div className="max-w-screen-xl mx-auto px-6 md:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map(f => (
              <div key={f.title} className="bg-white rounded-xl border border-slate-200 p-6 hover:shadow-md transition group">
                <div className="w-10 h-10 rounded-lg bg-emerald-50 flex items-center justify-center mb-4 group-hover:bg-emerald-100 transition">
      ...
      <section className="py-16 bg-white">
        <div className="max-w-xl mx-auto text-center px-6">
          <h2 className="text-2xl font-bold text-slate-900 mb-3">Ready to predict your crop yield?</h2>
          <p className="text-slate-500 text-sm mb-6">Create an account or sign in to access the full prediction system.</p>
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-slate-900">{type === "login" ? "Welcome back" : "Create your account"}</h1>
          <p className="text-slate-500 text-sm mt-1">{type === "login" ? "Sign in to access your prediction dashboard" : "Join the CropYieldAI prediction system"}</p>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 flex flex-col gap-4">
          ...
          <p className="text-center text-xs text-slate-400 mt-1">
            ...
            {" "}<button onClick={() => setPage(type === "login" ? "register" : "login")} className="text-emerald-700 font-semibold hover:underline">
              {type === "login" ? "Register" : "Sign in"}
            </button>
          </p>
          <div className="border-t border-slate-100 pt-3">
            <p className="text-xs text-slate-400 text-center mb-2">Or continue with demo credentials</p>
        ...
        <button onClick={() => setPage("landing")} className="mt-4 text-xs text-slate-400 hover:text-slate-600 flex items-center gap-1 mx-auto">
          ← Back to home
        </button>
      </div>
    </div>
...
const DashboardPage = ({ setPage, history, user }) => {
  ...
  return (
    <div className="flex-1 bg-slate-50 py-8 px-4 md:px-8">
      <div className="max-w-screen-xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-extrabold text-slate-900">Welcome back, {user?.name?.split(" ")[0]} 👋</h1>
          <p className="text-slate-500 text-sm mt-1">Here is a summary of your crop yield prediction activity.</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-bold text-slate-900">Recent Predictions</h2>
          ...
          {history.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-100 text-slate-500 text-xs uppercase">
                ...
                <tbody>
                  {history.slice(0, 5).map(h => (
                    <tr key={h.id} className="border-b border-slate-50 hover:bg-slate-50 transition">
                      <td className="py-3 text-slate-500">{h.date}</td>
                      <td className="py-3 font-semibold text-slate-800">{h.crop}</td>
                      <td className="py-3 text-slate-600">{h.region}</td>
                      <td className="py-3 text-right font-bold text-emerald-700">{h.predictedYield}</td>
...
const PredictPage = ({ onResult }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const result = await mockPredict(form);
      onResult({ ...form, ...result, date: new Date().toISOString().slice(0, 10), id: Date.now() });
    } finally { setLoading(false); }
  };

  const ch = handleChange;

  if (loading) return <div className="flex-1 bg-slate-50 flex items-center justify-center py-20"><div className="w-full max-w-md"><Spinner /></div></div>;

  return (
    <div className="flex-1 bg-slate-50 py-8 px-4 md:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="mb-7">
          <h1 className="text-2xl font-extrabold text-slate-900">Crop Yield Prediction</h1>
          <p className="text-slate-500 text-sm mt-1">Enter the agricultural parameters below to generate a machine learning-based yield estimate.</p>
        </div>

        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          {/* Section 1: Crop Info */}
          <div className="border-b border-slate-100 px-7 py-5">
            <p className="text-xs font-bold uppercase tracking-widest text-emerald-700 mb-4">01 — Crop & Location</p>
          ...
          <div className="border-b border-slate-100 px-7 py-5">
            <p className="text-xs font-bold uppercase tracking-widest text-emerald-700 mb-4">02 — Environmental Conditions</p>
          ...
          {/* Section 3: Agronomic */}
          <div className="border-b border-slate-100 px-7 py-5">
            <p className="text-xs font-bold uppercase tracking-widest text-emerald-700 mb-4">03 — Agronomic Inputs</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          ...
          <div className="px-7 py-5">
            <p className="text-xs font-bold uppercase tracking-widest text-emerald-700 mb-4">04 — Prediction Year</p>
          ...
          {/* Actions */}
          <div className="bg-slate-50 border-t border-slate-100 px-7 py-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <p className="text-xs text-slate-400 max-w-sm">All fields marked with <span className="text-red-500">*</span> are required. Ensure values are accurate for best prediction quality.</p>
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center">
            <Icon path={Icons.check} size={18} className="text-emerald-700" />
          </div>
          <div>
            <h1 className="text-xl font-extrabold text-slate-900">Prediction Complete</h1>
            <p className="text-xs text-slate-400">Generated on {result.date}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6">
          <h2 className="font-bold text-slate-900 mb-3">Model Interpretation</h2>
          ...
          <p className="text-slate-400 text-xs mt-3 border-t border-slate-50 pt-3">
        ...
        {/* Input Summary */}
        <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6">
          <h2 className="font-bold text-slate-900 mb-4">Input Summary</h2>
...
const HistoryPage = ({ history, setHistory, setPage, onViewResult }) => {
  ...
  return (
    <div className="flex-1 bg-slate-50 py-8 px-4 md:px-8">
      <div className="max-w-screen-xl mx-auto">
        <div className="mb-7">
          <h1 className="text-2xl font-extrabold text-slate-900">Prediction History</h1>
          <p className="text-slate-500 text-sm mt-1">A record of all crop yield predictions made on this system.</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="border-b border-slate-100 px-6 py-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            <h2 className="font-bold text-slate-900">All Records</h2>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by crop or region..."
              className="rounded-lg border border-slate-200 px-3.5 py-2 text-sm w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
          </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-100 text-slate-400 text-xs uppercase bg-slate-50">
                ...
                <tbody>
                  {filtered.map(h => (
                    <tr key={h.id} className="border-b border-slate-50 hover:bg-slate-50 transition">
                      <td className="px-5 py-3.5 text-slate-500">{h.date}</td>
                      <td className="px-5 py-3.5 font-semibold text-slate-800">{h.crop}</td>
                      <td className="px-5 py-3.5 text-slate-600">{h.region}</td>
                      <td className="px-5 py-3.5"><Badge color={h.season === "Wet" ? "blue" : "amber"}>{h.season}</Badge></td>
                      <td className="px-5 py-3.5 text-right font-bold text-emerald-700">{h.predictedYield}</td>
                      <td className="px-5 py-3.5"><Badge color="green">{h.status}</Badge></td>
                      <td className="px-5 py-3.5">
                        <div className="flex gap-2">
                          <button onClick={() => onViewResult(h)} className="p-1.5 rounded-lg hover:bg-emerald-50 text-emerald-700 transition" title="View details">
                            <Icon path={Icons.eye} size={15} />
                          </button>
                          <button onClick={() => setHistory(prev => prev.filter(p => p.id !== h.id))} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 transition" title="Delete">
...
const ModelPage = () => {
  ...
  return (
    <div className="flex-1 bg-slate-50 py-8 px-4 md:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="mb-8">
          <p className="text-emerald-700 font-semibold text-xs uppercase tracking-widest mb-2">Technical Documentation</p>
          <h1 className="text-2xl font-extrabold text-slate-900">Machine Learning Model Information</h1>
          <p className="text-slate-500 text-sm mt-1">Technical details of the predictive model used in this system.</p>
        </div>

        <div className="space-y-6">
          {/* Model Summary */}
          <div className="bg-white border border-slate-200 rounded-xl p-7">
            <h2 className="font-bold text-slate-900 text-lg mb-4 pb-3 border-b border-slate-100">Model Overview</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-5">
              {[["Algorithm", "Random Forest Regressor"], ["R² Score", "0.891"], ["RMSE", "1.24 t/ha"], ["MAE", "0.87 t/ha"], ["Training Samples", "~28,000"], ["Features Used", "11"]].map(([k, v]) => (
                <div key={k} className="bg-emerald-50 rounded-lg px-4 py-3">
          ...
          {/* Why RF */}
          <div className="bg-white border border-slate-200 rounded-xl p-7">
            <h2 className="font-bold text-slate-900 text-lg mb-4 pb-3 border-b border-slate-100">Why Random Forest?</h2>
            <div className="space-y-4 text-sm text-slate-600 leading-relaxed">
              <p><strong className="text-slate-800">Robustness to Overfitting:</strong> As an ensemble of decision trees trained on randomised subsets of data, Random Forests resist memorising noise in the training data. This is particularly important given the high variability in agricultural datasets.</p>
              <p><strong className="text-slate-800">Handles Mixed Input Types:</strong> The model natively handles both continuous (rainfall, temperature) and categorical (crop, soil type, season) features without requiring extensive preprocessing.</p>
              <p><strong className="text-slate-800">Feature Importance:</strong> Built-in feature importance rankings enable model transparency — a key requirement for academic and practical trustworthiness of the system.</p>
              <p><strong className="text-slate-800">Non-linearity:</strong> Agricultural yield relationships are highly non-linear. Random Forests capture complex interactions between rainfall and temperature without explicit feature engineering.</p>
            </div>
          </div>

          {/* Feature Importance */}
          <div className="bg-white border border-slate-200 rounded-xl p-7">
            <h2 className="font-bold text-slate-900 text-lg mb-5 pb-3 border-b border-slate-100">Feature Importance</h2>
            <div className="space-y-4">
              {features.map(f => (
                <div key={f.name}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-semibold text-slate-800">{f.name}</span>
                    <span className="text-xs font-bold text-emerald-700">{(f.importance * 100).toFixed(0)}%</span>
                  </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-7">
            <h2 className="font-bold text-amber-900 text-lg mb-4">Model Limitations</h2>
          ...
          {/* Use Cases */}
          <div className="bg-white border border-slate-200 rounded-xl p-7">
            <h2 className="font-bold text-slate-900 text-lg mb-4 pb-3 border-b border-slate-100">Expected Use Cases</h2>
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              {[
                ...
                <div key={t} className="bg-slate-50 rounded-lg p-4">
                  <p className="font-bold text-slate-800 mb-1">{t}</p>
...
const AboutPage = () => (
  <div className="flex-1 bg-slate-50 py-8 px-4 md:px-8">
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <p className="text-emerald-700 font-semibold text-xs uppercase tracking-widest mb-2">Project Documentation</p>
        <h1 className="text-2xl font-extrabold text-slate-900">About This Project</h1>
        <p className="text-slate-500 text-sm mt-1">A final year computer science research project on machine learning in agriculture.</p>
      </div>

      <div className="space-y-6">
        <div className="bg-white border border-slate-200 rounded-xl p-7">
          <h2 className="font-bold text-slate-900 text-lg mb-4 pb-3 border-b border-slate-100">Project Overview</h2>
          <p className="text-slate-600 text-sm leading-relaxed">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-7">
          <h2 className="font-bold text-slate-900 text-lg mb-4 pb-3 border-b border-slate-100">Problem Statement</h2>
          <p className="text-slate-600 text-sm leading-relaxed">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-7">
          <h2 className="font-bold text-slate-900 text-lg mb-3 pb-3 border-b border-slate-100">Aim & Objectives</h2>
          <p className="text-slate-600 text-sm leading-relaxed mb-4"><strong>Aim:</strong> To develop a machine learning-based web application capable of predicting crop yields for Nigerian agricultural regions with demonstrably high accuracy.</p>
          <p className="text-slate-700 text-sm font-semibold mb-2">Specific Objectives:</p>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-7">
          <h2 className="font-bold text-slate-900 text-lg mb-4 pb-3 border-b border-slate-100">Relevance to Nigerian Agriculture</h2>
          <p className="text-slate-600 text-sm leading-relaxed">
...
const HelpPage = () => {
  ...
  return (
    <div className="flex-1 bg-slate-50 py-8 px-4 md:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="mb-8">
          <p className="text-emerald-700 font-semibold text-xs uppercase tracking-widest mb-2">Support</p>
          <h1 className="text-2xl font-extrabold text-slate-900">Help & Guidance</h1>
          <p className="text-slate-500 text-sm mt-1">Answers to common questions and a guide to using the prediction system.</p>
        </div>

        {/* How to Use */}
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-7 mb-6">
          <h2 className="font-bold text-emerald-900 text-lg mb-4">How to Use the Prediction System</h2>
          <ol className="space-y-3">
            {[
              ...
              <li key={i} className="flex gap-4">
                ...
                <div>
                  <p className="font-semibold text-emerald-900 text-sm">{t}</p>
                  <p className="text-emerald-700 text-xs mt-0.5">{d}</p>
                </div>
              </li>
            ))}
          </ol>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-6">
          ...
          <div className="divide-y divide-slate-100">
            {faqs.map((f, i) => (
              <div key={i}>
                <button onClick={() => setOpen(open === i ? null : i)} className="w-full text-left flex items-center justify-between px-6 py-4 hover:bg-slate-50 transition">
                  <span className="text-sm font-semibold text-slate-800">{f.q}</span>
                  <Icon path={open === i ? Icons.close : Icons.plus} size={16} className="text-slate-400 shrink-0 ml-3" />
                </button>
                {open === i && (
                  <div className="px-6 pb-4 text-sm text-slate-600 leading-relaxed">{f.a}</div>
                )}
              </div>
            ))}
          </div>
        ...
        {/* Contact */}
        <div className="bg-white border border-slate-200 rounded-xl p-7">
          <h2 className="font-bold text-slate-900 text-lg mb-5 pb-3 border-b border-slate-100">Contact & Support</h2>
          {sent ? (
            <div className="text-center py-8">
              <div className="w-12 h-12 rounded-full bg-emerald-100 flex items-center justify-center mx-auto mb-3">
                <Icon path={Icons.check} size={22} className="text-emerald-700" />
              </div>
              <p className="font-bold text-slate-900">Message Sent</p>
              <p className="text-slate-500 text-sm mt-1">Thank you for your message. We'll respond as soon as possible.</p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} rows={4} placeholder="Describe your question or issue..."
                  className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
              </div>
...
export default function App() {
  ...
  const [history, setHistory] = useState(() => {
    try { const s = localStorage.getItem("cyai_history"); return s ? JSON.parse(s) : MOCK_HISTORY; } catch { return MOCK_HISTORY; }
  });
  ...
  const renderPage = () => {
    switch (page) {
      case "landing": return <LandingPage setPage={protectedSetPage} />;
      ...
      case "predict": return <PredictPage onResult={handleResult} />;
      ...
      default: return <LandingPage setPage={protectedSetPage} />;
    }
  };
